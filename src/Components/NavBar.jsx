...
function NavBar({ link, searchBar, display, home,commerce }) {
  const {
    ProductData,
    homeSelectedInsert,
  ...
  return (
    <div className="flex my-10 py-8 justify-between items-center w-full p-6 h-20 ">
      {link && (
        <Link to={`/${link}`}>
      ...
      <div className="h-fit     ">
        <div className="w-[350px] relative py-2 pr-6 p-2 flex gap-3 items-center h-full rounded-full  bg-card">
          <img src={localStorage.getItem('img')} className="w-20 object-cover h-20 bg-main rounded-[50%] " />
          <div className="px-1 py-2">
            <h1 className="font-medium"> {localStorage.getItem("name")} </h1>
            <p className="text-xs"> {role[localStorage.getItem('role')]} </p>
          </div>
          <button>
            <img src={exit} onClick={()=>HandleLogOut()}  className="w-8 absolute right-4 top-8 rotate-180" />
          </button>
        </div>
      </div>
    </div>
